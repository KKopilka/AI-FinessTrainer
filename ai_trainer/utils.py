...
#     """Undo padding from keypoints coordinates.

#     Args:
#         keypoints (np.ndarray): array of keypoints with shape (N, 5) where N is
#             the number of keypoints and each keypoint has 5 elements (x, y, z,
#             probability, visibility).
#         original_image_size (Tuple[int, int]): tuple containing the original
#             height and width of the image.

#     Returns:
#         np.ndarray: array of keypoints with coordinates adjusted to the original
#             image size.
#     """
#     new_keypoints = keypoints.copy()
#     height, width = original_image_size
#     new_keypoints[..., 0] *= width
#     new_keypoints[..., 1] *= height
#     return new_keypoints


# def normalize(image: np.ndarray, floor: float, ceil: float) -> np.ndarray:
#     """Normalize image pixel values to the range [floor, ceil].
...
#     """
#     normalized_image = np.clip(image, floor, ceil)
