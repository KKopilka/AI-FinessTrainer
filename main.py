...
#     def squats(self):
#         for i in utilities().illustrate_exercise(
#             "asserts/squats_illustration.jpeg", "SQUATS"
#         ):
#             yield (i)
...
#     def complete_path(self, difficulty_level):
...
from ai_trainer.models import BlazePoseModel
from ai_trainer.drawing import draw_pose, draw_text
from ai_trainer.feedback.front_squat import give_feedback, find_angle2
from AudioCommSys import *

def find_angle2(p1, p2, p3, draw=True, image=None):
    # Get the landmarks
    x1, y1 = p1[1:]
    x2, y2 = p2[1:]
    x3, y3 = p3[1:]
    ...
    if draw and image is not None:
        ...
        cv2.putText(image, f'Angle: {angle:.2f}', (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    return angle
...
def main():
    ...
    while cap.isOpened():
        ...
        if not all(probs<0):
            ...
            p2=kps[25]
            p3=kps[27]
            # Вызов функции find_angle2 для подписи угла на изображении
            angle = find_angle2(p1, p2, p3, draw=True, image=frame)
